...
var prompts = /*@__PURE__*/getDefaultExportFromCjs(prompts$1);
...
function repeatFactory(confirmMsg, textMsg) {
  ...
  return async function repeat_confirm_text(name, count = 0) {
    ...
    const targetPath = path.resolve(process$1.cwd(), name);
...
async function fileBlob(catalogItem, configFile, parse) {
  ...
  spinner.stop();
  const filePath = path.resolve(process$1.cwd(), downloadRelativeDest);
...
async function addAction$1(configFile, _args) {
  const [path] = _args;
  if (!path)
...
var addAction = errorWrapper(addAction$1);

let coutLevel = 0;
async function getListAction$1(configFile, _args) {
...
var getListAction = errorWrapper(getListAction$1);

const CNONFIG_FILE_LIST = ["dlc.config.ts", "dlc.config.js"];
const defualtParse = async (path2, data) => {
  const content = node_buffer.Buffer.from(data.content, "base64");
